          ...
          <?php foreach ($pendaftaran_kk_perceraian as $kkper) : ?>
            <tr>
              ...
              <td>
                ...
                <form action="<?= base_url('/DetailAdmin/detail_pendaftarankkperceraian_admin/' . $kkper['namapemohon']); ?>" method="post" class="d-inline">
                  <?= csrf_field(); ?>
                ...
                <button class="btn btn-success btn-sm">
                  <a href="https://api.whatsapp.com/send/?phone=<?= $kkper['nomorpemohon'] ?>&text=Assalamu%27alaikum%2C+perkenalkan+nama+saya.....&type=phone_number&app_absent=0" class="bi bi-whatsapp text-white" data-placement="top" title="Kirim Whatsapp"> Kirim WA</a>
                </button>

                <form action="<?= base_url('DeleteAdmin/tandaiSelesaiKKPerceraian/' . $kkper['id']); ?>" method="post" class="d-inline">
                  <?= csrf_field(); ?>
                  <?php if ($kkper['status'] === 'Belum di Proses') : ?>
                  ...
                  <?php elseif ($kkper['status'] === 'Gagal Verifikasi' || $kkper['status'] === 'Selesai Verifikasi') : ?>
