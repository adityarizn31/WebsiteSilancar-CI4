          ...
          <?php foreach ($perbaikan_data as $perda) :  ?>
            <tr>
              ...
              <td>
                ...
                <form action="<?= base_url('/DetailAdmin/detail_pendaftaranperbaikandata_admin/' . $perda['namapemohon']); ?>" method="post" class="d-inline">
                  <?= csrf_field(); ?>
                ...
                <button class="btn btn-success btn-sm">
                  <a href="https://api.whatsapp.com/send/?phone=<?= $perda['nomorpemohon'] ?>&text=Assalamu%27alaikum%2C+perkenalkan+nama+saya.....&type=phone_number&app_absent=0" class="bi bi-whatsapp text-white" data-placement="top" title="Kirim Whatsapp"> Kirim WA</a>
                </button>

                <form action="<?= base_url('DeleteAdmin/tandaiSelesaiPerbaikanData/' . $perda['id']); ?>" method="post" class="d-inline">
                  <?= csrf_field(); ?>
                  <?php if ($perda['status'] === 'Belum di Proses') : ?>
                  ...
                  <?php elseif ($perda['status'] === 'Gagal Verifikasi' || $perda['status'] === 'Selesai Verifikasi') : ?>
